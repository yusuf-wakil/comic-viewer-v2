...
export function getComicById(db: Database.Database, id: string): Comic | null {
  const row = db.prepare('SELECT * FROM comics WHERE id = ?').get(id) as
...
export function getComicByPath(db: Database.Database, filePath: string): Comic | null {
  const row = db.prepare('SELECT * FROM comics WHERE path = ?').get(filePath) as
    | Record<string, unknown>
    | undefined
  return row ? rowToComic(row) : null
}
...
export function getAllComics(db: Database.Database): Comic[] {
  const rows = db.prepare('SELECT * FROM comics ORDER BY series, issue_number').all() as Record<
    string,
    unknown
  >[]
...
export function getProgress(db: Database.Database, comicId: string): ReadingProgress | null {
  const row = db.prepare('SELECT * FROM reading_progress WHERE comic_id = ?').get(comicId) as
    | Record<string, unknown>
    | undefined
  if (!row) return null
  return {
